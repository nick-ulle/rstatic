...
The remainder of this section is a reference on the subclasses of `ASTNode`.
...
`to_astq()` function is like `to_ast()`, but automatically quotes its argument.
```{r}
tau = to_astq(6.28)
...
fields `read` and `write`, and respective `set_` methods. The `write` field
holds the left-hand side of the assignment, and the `read` field holds the
right-hand side.

As an example, consider the code below, which assigns the complex number `3+2i`
to the variable `z`.
```{r}
assign = to_astq(z <- 3+2i)
```
...
All `Application`s have an `args` field and `set_args()` method. The `args`
field holds the list of arguments. For `call_sum`, this is an empty list.
```{r}
call_sum$args
```

It's not a good idea to set `args` on an `Application` directly, because each
node in the argument list needs to have its `parent` field set to the
`Application`. The `set_args()` function takes care of setting `parent` fields,
so it's the preferred method for setting `args`.

We can set the sum in the example to have two arguments, the integers `4` and
`6`.
```{r}
args = list(Integer$new(4), Integer$new(6))
call_sum$set_args(args)

call_sum
```

In addition to the members inherited from `Application`, `Call`s also have a
`fn` field and `set_fn()` method. The `fn` field holds the function being
called, either as a `Symbol` or as a `Function` (for calls to anonymous
functions).
...
The `Replacement` class represents a call to a replacement functions. This
class does not add any new members, but changes the `set_fn()` method to make
sure the function name `fn` ends with `"<-"`, which is required for all
...
has fields `body` and `is_paren`, and method `set_body()`. The `is_paren` field
is `TRUE` if the `Brace` object represents parentheses and `FALSE` if the
object represents curly braces.
...
As usual, the `set_body()` method should be used to set the `body` field rather
than setting the field directly.

### Functions & Primitives

Callable objects are represented by the `Callable` class. The `Callable` class
defines a `params` field and corresponding `set_params()` method. The `params`
field holds a list of parameters (and default arguments) for the callable
object.

`Function` and `Primitive`, both subclasses of `Callable`, represent functions
and primitives, respectively. The `Function` class adds a `body` field and
`set_body()` method to the members inherited from `Callable`. The `body` field
holds the single expression inside the function; for functions that contain
more than one expresion, `body` holds a `Brace` object.
...
class adds a `fn` field and `set_fn()` method to the members inherited from
`Callable`. The `fn` field holds the name of the primitive, and is similar to
the `fn` field on the `Call` class.
...
The `If` class has fields `condition`, `true`, and `false`, with corresponding
`set_` methods. These fields hold to the if-statement's condition, "if" branch,
and "else" branch, respectively. The `false` field is `NULL` for if-statements
that don't have an "else" branch.
...
The `For` class has fields `ivar`, `iter`, and `body`, along with respective
`set_` methods. The `ivar` field holds the iteration variable, the `iter` field
holds the object being iterated over, and the `body` field holds the list of
expressions that make up the body of the loop.

The `While` class has fields `condition`, `body`, and `is_repeat`. There are
also set methods `set_condition()` and `set_body()`. Similar to the `If` class,
the `condition` field holds the condition for the loop. The `body` field holds
the list of expressions that make up the body of the loop. The `is_repeat`
field is `TRUE` if the loop is a `repeat` loop and `FALSE` if the loop is a
`while` loop. When `is_repeat` is `TRUE`, the `condition` field is ignored (and
defaults to `NULL`).
